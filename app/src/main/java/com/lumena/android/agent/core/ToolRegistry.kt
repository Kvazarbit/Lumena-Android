...
object ToolRegistry {
    private val specs = listOf(
        ...
        ToolSpec("file.list", ToolRisk.READ_ONLY, description = "List a directory in an allowed read root. Optional path; use aliases exposed by workspace.list/context.snapshot, such as @shared or @btc."),
        ToolSpec("file.search", ToolRisk.READ_ONLY, setOf("query"), "Search file names and text in an allowed read root. Optional path selects an alias exposed by workspace.list/context.snapshot; search each relevant root deliberately."),
        ...
        ToolSpec("file.read", ToolRisk.READ_ONLY, setOf("path"), "Read a text file from the workspace or an explicit read-only alias such as @shared/path. Optional start_line/end_line are 1-based inclusive."),
