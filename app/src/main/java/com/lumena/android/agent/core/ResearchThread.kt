 ...
 * paths or prose. Explicit user scope wins; only a clearly related follow-up
 * may carry an already-verified previous project id forward.
 */
object ProjectContextResolver {
    ...
    internal fun normalizeProjectId(raw: String): String? {
        ...
        if (!Regex("""[A-Za-z0-9][A-Za-z0-9._-]{0,79}""").matches(value)) {
            return null
        }
        return value
    }
