...
object ResearchThreadResolver {
    ...
    private val sourceIdentityTerms = Regex(
        "(?iu)(?:" +
            "\\b(?:яке|який|яка|що\\s+за)\\s+(?:джерел[оа]?|ресурс|сайт|посилання)\\b|" +
            "\\b(?:джерело|ресурс|сайт|посилання)\\s*\\??$|" +
            "\\bзвідки\\s+(?:це|ця|цей|дані|інформац[іяї])\\b|" +
            "\\bwhat\\s+(?:source|site|resource|link)\\b|" +
            "\\bwhere\\s+(?:is\\s+)?(?:this|that)\\s+from\\b|" +
            "\\b(?:source|site|resource|link)\\s*\\??$|" +
            "\\b(?:какой|что\\s+за)\\s+(?:источник|ресурс|сайт|ссылка)\\b|" +
            "\\b(?:источник|ресурс|сайт|ссылка)\\s*\\??$|" +
            "\\bоткуда\\s+это\\b|" +
            "\\b(?:jakie|które|co\\s+to\\s+za)\\s+(?:źródło|strona|serwis|link)\\b|" +
            "\\bskąd\\s+to\\b" +
        ")"
    )
