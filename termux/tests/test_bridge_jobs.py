...
class JobsTest(unittest.TestCase):
    def test_live_output_cancel_and_replay(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            rid = 'test-live-output-123'; payload = {'tool': 'python.run'}; result = []
            action = lambda: r.run_process([sys.executable, '-c', 'import time;print("READY",flush=True);time.sleep(30)'], Path(d))
            worker = threading.Thread(target=lambda: result.append(r.execute(rid, payload, action)))
            worker.start(); until = time.monotonic() + 5
            while 'READY' not in r.status(rid).get('stdout', '') and time.monotonic() < until: time.sleep(.02)
            self.assertIn('READY', r.status(rid)['stdout'])
            r.cancel(rid); worker.join(6)
            self.assertFalse(worker.is_alive()); self.assertEqual('cancelled', r.status(rid)['status'])
    ...
    def test_cancel_before_request(self):
        r=JobRegistry(); rid='cancel-before-arrival'; r.cancel(rid)
    ...
    def test_sigterm_ignored(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            t=threading.Thread(target=lambda:result.append(r.execute(rid,{},action)));t.start()
            until=time.monotonic()+5
            while 'READY' not in r.status(rid).get('stdout','') and time.monotonic()<until: time.sleep(.02)
            r.cancel(rid);t.join(7)
            self.assertFalse(t.is_alive());self.assertFalse(result[0]['ok'])
    def test_capacity_keeps_cancellation_tombstone(self):
        r=JobRegistry(capacity=1); r.cancel('keep-this-request-123')
        with self.assertRaises(ValueError): r.cancel('different-request-123')
        self.assertEqual('cancelled',r.status('keep-this-request-123')['status'])
